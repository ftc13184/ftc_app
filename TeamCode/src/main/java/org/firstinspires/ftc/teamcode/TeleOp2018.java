...
public class TeleOp2018 extends LinearOpMode{
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            // Use gamepad right Bumpers to open and close the claw
            if (gamepad1.right_bumper)
                clawOffset = clawOffset + CLAW_SPEED;

            // Move both servos to new position.  Assume servos are mirror image of each other.
            clawOffset = Range.clip(clawOffset, -0.5, 0.5);
            //robot.leftTwist.setPosition(robot.MID_SERVO + clawOffset);
            //robot.rightTwist.setPosition(robot.MID_SERVO + clawOffset);

            // Use gamepad buttons to move arm up (Y) and down (A)
            if (gamepad1.y)
                robot.linearLatch.setPower(robot.ARM_UP_POWER);
            else
                robot.linearLatch.setPower(0.0);
